import React from 'react'
import SellerAccountForm from './SellerAccountForm';
import SellerLoginForm from './SellerLoginForm';
import { Button } from '@mui/material';

const BecomeSeller = () => {
    const [isLogin, setIsLogin] = React.useState(false);

    const handleShowPage = () => {
        setIsLogin(!isLogin);
    }

    return (
        <div className='grid md:gap-10 grid-cols-3 min-h-screen'>
            <section className='lg:col-span-1 md:col-span-2 col-span-3 p-10 shadow-lg rounded-b-md'>
                {isLogin ? <SellerAccountForm /> : <SellerLoginForm />}

                <div className='mt-10 space-y-2'>
                    <h1 className='text-center text-sm font-medium'>Have Account</h1>
                    <Button onClick={handleShowPage} fullWidth sx={{ py: "11px" }} variant='contained'>
                        {isLogin ? "Register New Account" : "Login Existing Account"}
                    </Button>
                </div>
            </section>
            <section className="md:col-span-1 lg:col-span-2 hidden md:flex justify-center items-center">
                <div className='lg:w-[80%] px-5 space-y-10'>
                    <div className='space-y-2 font-bold text-center'>
                        <p className='text-2xl'>Join the Marketplace Revolution</p> 
                        <p className='text-lg text-primary-color'>Boost your sales today</p>
                    </div>
                    <div className='h-full w-full flex justify-center'>
                        <img src="https://img.freepik.com/free-vector/seller-concept-professional-worker-supermarket-shop-store-stocktacking-merchandising-cash-accounting-calculations-client-service-payment-operation-vector-illustration_613284-3114.jpg" alt='Boost your sales today'/>
                    </div>
                </div>
            </section>
        </div>
    )
}

export default BecomeSeller